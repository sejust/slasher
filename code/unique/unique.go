...
func initPath() {
	...
	if os.IsNotExist(err) {
		err = os.WriteFile(_nameDB, []byte{'[', ']'}, 0o755)
	}
	panicIfError(_nameDB, err)

	_, err = os.Stat(_nameConf)
	if os.IsNotExist(err) {
		...
		err = os.WriteFile(_nameConf, b, 0o755)
	}
...
func Run() {
	...
	go func() {
		...
		for _, dir := range config.Dirs {
			var trim string
			fullDir, err := filepath.Abs(dir.Dir)
			panicIfError(dir.Dir, err)
			switch dir.KeepMode {
			case "base":
	...
	data, _ = json.MarshalIndent(filesSave, "", "    ")
